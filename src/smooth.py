...
def smooth_predict_hard_derandomized(model, x, noise,  noise_batch_size=512):
    ...
    """
    counts = None
    tranched_counts = None
    num_samples_left = noise.total_samples_count()
    if ('tranches' in dir(noise)):
        num_tranches = noise.num_tranches()
        tranch_weights = noise.tranch_weights()
    while num_samples_left > 0:
        ...
        if (noise.doubles_channels()):
            flattened_shape = torch.Size([flattened_shape[0],]) + torch.Size([flattened_shape[1]*2,]) + flattened_shape[2:]
        ...
        if ('tranches' in dir(noise)):
            if tranched_counts is None:
                tranched_counts = torch.zeros(x.shape[0], num_tranches, logits.shape[-1], dtype=torch.float, device=x.device)
            tranches = noise.tranches(start_sample,end_sample)
            tranched_counts += (F.one_hot(tranches,num_tranches).unsqueeze(2).unsqueeze(0) * F.one_hot(top_cats, logits.shape[-1]).float().unsqueeze(2)).sum(dim=1)
        else:
            if counts is None:
                counts = torch.zeros(x.shape[0], logits.shape[-1], dtype=torch.float, device=x.device)
            counts += F.one_hot(top_cats, logits.shape[-1]).float().sum(dim=1)
        num_samples_left -= noise_batch_size
    if ('tranches' in dir(noise)):
         weighted_counts = (tranched_counts* tranch_weights.unsqueeze(0).unsqueeze(2)).sum(dim =1)
         return weighted_counts,tranched_counts
    else:
        return counts
