...
from src.lib.wide_resnet import DoubleWideResNet as DoubleWideResNetBase
from src.lib.double_resnet import double_resnet50 as double_resnet50
...
class DoubleForecaster(nn.Module):

    def __init__(self, dataset, device):
        super().__init__()
        self.device = device
        nzshape = get_normalization_shape(dataset)
        nzstats  = get_normalization_stats(dataset)
        nzstats = {"mu":nzstats["mu"]*2, "sigma":nzstats["sigma"]*2}
        self.norm = NormalizeLayer((nzshape[0]*2,) + nzshape[1:], device,
                                   **nzstats)
...
class DoubleResNet(DoubleForecaster):

    def __init__(self, dataset, device):
        super().__init__(dataset, device)
        assert(dataset == "imagenet")
        self.model = nn.DataParallel(double_resnet50(num_classes=1000))
        self.norm = nn.DataParallel(self.norm)
        self.norm = self.norm.to(device)
        self.model = self.model.to(device)
...
class DoubleWideResNet(DoubleForecaster):
    
    def __init__(self, dataset, device):
        super().__init__(dataset, device)
        self.model = nn.DataParallel(DoubleWideResNetBase(depth=40, widen_factor=2,
                                                    num_classes=get_num_labels(dataset)))
        self.norm = nn.DataParallel(self.norm)
        self.norm = self.norm.to(device)
        self.model = self.model.to(device)
...
class WideResNetCompat(Forecaster):
    
    def __init__(self, dataset, device):
        super().__init__(dataset, device)
        norm_stats = get_normalization_stats(dataset)
        self.norm =  NormalizeLayeCompat(norm_stats['mu'],norm_stats['sigma'])
        self.model = nn.DataParallel(WideResNetBase(depth=40, widen_factor=2,
                                                    num_classes=get_num_labels(dataset)))
        self.norm = nn.DataParallel(self.norm)
        self.norm = self.norm.to(device)
...
class NormalizeLayeCompat(torch.nn.Module):
    ...
    def __init__(self, means, sds):
        ...
        """
        super(NormalizeLayeCompat, self).__init__()
