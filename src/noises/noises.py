...
class SplitMethodDerandomized(Noise):
    ...
    def __init__(self, dim, seed = 0,sigma=None, lambd=None,qs=None, s =None, device='cpu'):
        if  (s):
            lambd =s/2
        if (qs):
            lambd = qs/510.
    ...
    def classify_and_certify_l_1_over_2_exact_from_counts(self, counts):
        torchidx,  certs  = self.classify_and_certify_l1_exact_from_counts(counts)
        certs = torch.maximum(certs, certs**2)
        return torchidx, certs

    def classify_and_certify_l_1_over_3_exact_from_counts(self, counts):
        torchidx,  certs  = self.classify_and_certify_l1_exact_from_counts(counts)
        certs = torch.maximum(certs, certs**3)
        return torchidx, certs

class GlobalLambda(Noise):
    '''Split range, from 0.5+lambda to 0.5-lambda
    '''


    def __init__(self, dim, seed = 0,sigma=None, lambd=None, s =None, output_mode = 'center',device='cpu'):
        assert(s==1)
        if  (s):
            lambd =s/2
        super().__init__(dim, sigma, lambd, device)
        self.seed = seed
        assert output_mode in ['center', 'center_center', 'center_error', 'upper_lower']
        self.output_mode = output_mode
        self.templates = []
        for levels in range(1,256):
            rs = np.random.RandomState(seed=self.seed)
            self.templates.append(torch.tensor(rs.randint(levels, size=self.dim)).to(self.device).reshape(1,self.dim))

        self.templates = torch.stack(self.templates)
        self.tranch_weights_ = self._tranch_weights()
        self.rnd_tranch_weights = (self.tranch_weights_*torch.arange(1,256,device=self.device)).cpu().numpy()

    def __str__(self):
        return f"GlobalLambda(seed={self.seed}, dim={self.dim})"

    def plotstr(self):
        return 'GlobalLambda'

    def tabstr(self, adv):
        return f'global_lambda_{adv}_d{self.dim}'

    def _sigma(self):
        return 3 ** -0.5
    def doubles_channels(self):
        return self.output_mode in ['center_center', 'center_error', 'upper_lower']

    def sample(self, x): # For training only, is random
        levels = np.random.choice(255, p=self.rnd_tranch_weights, size = (x.shape[0],1)) +1
        offset = torch.tensor(np.random.randint(low=0,high=levels),device=self.device)
        levels = torch.tensor(levels,device=self.device)
        split_values = ((self.templates[levels.squeeze(1)-1].squeeze(1) + offset) % levels + 0.5)/255.0 

        ceil_dist = torch.ceil(255.*(x-split_values)/(levels))*levels/255. + split_values
        upper = torch.clamp(ceil_dist,max =1)
        lower = torch.clamp(ceil_dist - levels/255. ,min =0)
        if self.output_mode == 'center':
            return (upper + lower)/2
        elif self.output_mode == 'center_center':
            return ((upper + lower)/2).repeat(1,2)
        elif self.output_mode == 'center_error':
            return torch.cat([(upper + lower)/2, (upper-lower)/2],dim=1)
        elif self.output_mode == 'upper_lower':
            return torch.cat([upper, lower],dim=1)
    def total_samples_count(self):
        return 255*128
    def tranches(self,start_sample, end_sample):
        out = []
        if (end_sample is None):
            end_sample = 255*128
        for levels in range(1,256):
            if (start_sample >= levels):
                start_sample -= levels
                end_sample -= levels
                continue
            next_start = 0
            next_end = end_sample - levels
            end_sample = min(end_sample, levels)
            out.extend([levels-1]*(end_sample-start_sample))
            if (next_end <= 0):
                break
            else:
    ...
    def num_tranches(self):
        return 255
    def tranch_weights(self):
        return self.tranch_weights_
    def _tranch_weights(self):
        quantization = 255
        func = lambda x: min(math.sqrt(x),1)
        fn = np.array([func(x/quantization) for x in range(1,quantization + 1)])
        padded_fn = np.zeros(quantization+2)
        padded_fn[1:-1] = fn
        x = 2*fn - padded_fn[2:] - padded_fn[:-2] 
        x[-1] -= fn [-1]
    ...
    def sample_all_deterministic(self, x, start_sample = 0, end_sample = None): # Returns all possible distortions, output dim is (batch * distortions*dim)
        ...
        for levels in range(1,256):
            ...
            split_values = ((torch.arange(start_sample, end_sample, device=self.device).reshape(end_sample-start_sample,1)+self.templates[levels-1]).unsqueeze(0) % levels + 0.5)/255.0
            ceil_dist = torch.ceil(255.*(x.unsqueeze(1)-split_values)/(levels))*levels/255. + split_values
            upper = torch.clamp(ceil_dist,max =1)
            lower = torch.clamp(ceil_dist - levels/255. ,min =0)
            if self.output_mode == 'center':
                out.append( (upper + lower)/2)
            elif self.output_mode == 'center_center':
                out.append(((upper + lower)/2).repeat(1,1,2))
            elif self.output_mode == 'center_error':
                out.append(torch.cat([(upper + lower)/2, (upper-lower)/2],dim=2))
            elif self.output_mode == 'upper_lower':
                out.append(torch.cat([upper, lower],dim=2))
            if (next_end <= 0):
                break
            else:
                start_sample = next_start
    ...
    def classify_and_certify_l_1_over_2_exact_from_weighted_counts(self, weighted_counts):
        predinctionsnp = weighted_counts.cpu().numpy()
        idxsort = np.argsort(-predinctionsnp,axis=1,kind='stable')
        valsort = -np.sort(-predinctionsnp,axis=1,kind='stable')
        val =  valsort[:,0]
        idx = idxsort[:,0]
        valsecond =  valsort[:,1]
        idxsecond =  idxsort[:,1] 
        diffs = ((val - valsecond - (idxsecond <= idx)*3.867420964809298e-06)/2)
        certs =  ( torch.tensor(diffs).to(self.device))**2
        torchidx = torch.tensor(idx).to(self.device)
        return torchidx, certs



class ConcaveSplitMethodDerandomized(Noise):



    def __init__(self, dim, seed = 0,lambdas_2_q=None, ablates = 0, output_mode = 'center', device='cpu'):
        self.seed = seed
        self.device = device
        self.dim = dim
        self.lambdas_2_q = lambdas_2_q
        self.ablates =ablates
        assert output_mode in ['center', 'center_center', 'center_error', 'upper_lower']
        self.output_mode = output_mode
        self.portions_ranges = torch.cat([torch.arange(i,device=self.device) for i in lambdas_2_q])
        self.portions_sizes = torch.cat([torch.ones(i,device=self.device)*i for i in lambdas_2_q])
        self.portions_ranges = torch.cat((self.portions_ranges,torch.ones(ablates,device=self.device)*255))
        self.portions_sizes = torch.cat((self.portions_sizes,torch.ones(ablates,device=self.device)*256))
        self.levels = len(self.portions_ranges)
        rs = np.random.RandomState(seed=self.seed)
        self.template = torch.tensor(rs.randint(self.levels, size=self.dim)).to(self.device).reshape(1,self.dim)
    def __str__(self):
        return f"ConcaveSplitMethodDerandomized(seed={self.seed}, dim={self.dim}, lambdas_2_q={self.lambdas_2_q}, ablates={self.ablates})"

    def plotstr(self):
        return 'ConcaveSplitMethodDerandomized'

    def tabstr(self, adv):
        return f'concave_split_derandomized_{adv}_d{self.dim}'
    ...
    def sample(self, x): # For training only, is random
        offset = torch.randint(self.levels,(x.shape[0],1), device=self.device)
        level_ranges =self.portions_sizes[(offset+self.template)% self.levels]
        split_values = (self.portions_ranges[(offset+self.template)% self.levels] + 0.5)/255.0

        ceil_dist = torch.ceil(255.*(x-split_values)/(level_ranges))*level_ranges/255. + split_values
        upper = torch.clamp(ceil_dist,max =1)
        lower = torch.clamp(ceil_dist - level_ranges/255. ,min =0)
        if self.output_mode == 'center':
            return (upper + lower)/2
        elif self.output_mode == 'center_center':
            return ((upper + lower)/2).repeat(1,2)
    ...
    def sample_all_deterministic(self, x, start_sample = 0, end_sample = None): # Returns all possible distortions, output dim is (batch * distortions*dim)
        if (end_sample is None):
            end_sample = self.levels
        level_ranges =self.portions_sizes[(torch.arange(start_sample, end_sample, device=self.device).reshape(end_sample-start_sample,1)+self.template).unsqueeze(0)% self.levels]
        split_values = (self.portions_ranges[(torch.arange(start_sample, end_sample, device=self.device).reshape(end_sample-start_sample,1)+self.template).unsqueeze(0)% self.levels] + 0.5)/255.0
        ceil_dist = torch.ceil(255.*(x.unsqueeze(1)-split_values)/(level_ranges))*level_ranges/255. + split_values
        upper = torch.clamp(ceil_dist,max =1)
        lower = torch.clamp(ceil_dist - level_ranges/255. ,min =0)
        if self.output_mode == 'center':
            return (upper + lower)/2
        elif self.output_mode == 'center_center':
            return ((upper + lower)/2).repeat(1,1,2)
        elif self.output_mode == 'center_error':
            return torch.cat([(upper + lower)/2, (upper-lower)/2],dim=2)
        elif self.output_mode == 'upper_lower':
            return torch.cat([upper, lower],dim=2)

class L_1_over_2_SplitMethodDerandomized(ConcaveSplitMethodDerandomized):
    def __init__(self, dim, seed = 0, s = 1., output_mode = 'center', device='cpu'):
        lambdas_2_q = [1]*17 + [4] * 17 + [14]*2 + [17,  23,  40,  47,  67, 169, 254, 255]
        ablates = 15 +  int(math.ceil(1000 * (s-1)))
        self.s = s
        super().__init__(dim,  seed= seed, lambdas_2_q=lambdas_2_q, ablates = ablates, output_mode = output_mode, device = device)
    def classify_and_certify_l_1_over_2_exact_from_counts(self, counts):
        predinctionsnp = counts.cpu().numpy()
        idxsort = np.argsort(-predinctionsnp,axis=1,kind='stable')
        valsort = -np.sort(-predinctionsnp,axis=1,kind='stable')
        ...
        idxsecond =  idxsort[:,1] 
        diffs = ((val - valsecond - (idxsecond <= idx))/2).astype(int)
        certs =  (self.s  * torch.tensor(diffs).to(self.device)/float(self.levels))**2
        torchidx = torch.tensor(idx).to(self.device)
        return torchidx, certs


#>>> ind = [ 1,   2,   9,  15,  22,  37,  48,  93, 206, 255]
#>>> mult = [81,38,11,1,1,1,2,1,1,1]
class L_1_over_3_SplitMethodDerandomized(ConcaveSplitMethodDerandomized):
    def __init__(self, dim, seed = 0, s = 1., output_mode = 'center', device='cpu'):
        lambdas_2_q = [1]*81 + [2] * 38 + [9]*11 + [15,  22,  37, 48,48,93, 206, 255]
        ablates = 20 +  int(math.ceil(1000 * (s-1)))
        self.s = s
        super().__init__(dim,  seed= seed, lambdas_2_q=lambdas_2_q, ablates = ablates, output_mode = output_mode, device = device)
    def classify_and_certify_l_1_over_3_exact_from_counts(self, counts):
        predinctionsnp = counts.cpu().numpy()
        ...
        idxsecond =  idxsort[:,1] 
        diffs = ((val - valsecond - (idxsecond <= idx))/2).astype(int)
        certs =  (self.s  * torch.tensor(diffs).to(self.device)/float(self.levels))**3
        torchidx = torch.tensor(idx).to(self.device)
        return torchidx, certs


class L_0_SplitMethodDerandomized(ConcaveSplitMethodDerandomized):
    def __init__(self, dim, seed = 0, s = 1,  output_mode = 'center', device='cpu'):
        lambdas_2_q = [1]
        assert (s == int(s))
        s = int(s)
        ...
        self.s = s
        super().__init__(dim,  seed= seed, lambdas_2_q=lambdas_2_q, ablates = ablates, output_mode = output_mode, device = device)
    def classify_and_certify_l_0_exact_from_counts(self, counts):
        predinctionsnp = counts.cpu().numpy()
        idxsort = np.argsort(-predinctionsnp,axis=1,kind='stable')
        valsort = -np.sort(-predinctionsnp,axis=1,kind='stable')
        val =  valsort[:,0]
        ...
        idxsecond =  idxsort[:,1] 
        diffs = ((val - valsecond - (idxsecond <= idx))/2).astype(int)
        certs =  torch.tensor(diffs).to(self.device)
        torchidx = torch.tensor(idx).to(self.device)
        return torchidx, certs

class L_0_Pixel_SplitMethodDerandomized(L_0_SplitMethodDerandomized):
    def __init__(self, dim, seed = 0, s = 1,  output_mode = 'center', device='cpu'):
        super().__init__(dim,  seed= seed,  s= s, output_mode = output_mode, device = device)
        rs = np.random.RandomState(seed=self.seed)
        channeldim = self.dim//3
        self.template = torch.tensor(rs.randint(self.levels, size=channeldim)).to(self.device).repeat(3).reshape(1,self.dim)

class ConcaveSplitMethodDerandomizedPermute(Noise):
    ...
    def __init__(self, dim, seed = 0,lambdas_2_q=None, ablates = 0, output_mode = 'center', device='cpu'):
        ...
        assert output_mode in ['center', 'center_center', 'center_error', 'upper_lower']
        self.output_mode = output_mode
        self.portions_ranges = torch.cat([torch.arange(i,device=self.device) for i in lambdas_2_q]) # s's
        self.portions_sizes = torch.cat([torch.ones(i,device=self.device)*i for i in lambdas_2_q]) # Lambdas
        self.portions_ranges = torch.cat((self.portions_ranges,torch.ones(ablates,device=self.device)*255))
        self.portions_sizes = torch.cat((self.portions_sizes,torch.ones(ablates,device=self.device)*256))
        stacked_lambda_ss = torch.stack([self.portions_ranges,self.portions_sizes]).cpu().numpy()
        self.levels = len(self.portions_ranges)
        rs = np.random.RandomState(seed=self.seed)
        all_ss = []
        all_lambdas = []
        for i in range(dim):
            stacked_permuted_lambda_ss =np.transpose(rs.permutation(np.transpose(stacked_lambda_ss)))
            all_ss.append(stacked_permuted_lambda_ss[0])
            all_lambdas.append(stacked_permuted_lambda_ss[1])
        self.all_ss = torch.tensor(all_ss,device=self.device).permute(1,0)
        self.all_lambdas = torch.tensor(all_lambdas,device=self.device).permute(1,0)
    def __str__(self):
        return f"ConcaveSplitMethodDerandomizedPermute(seed={self.seed}, dim={self.dim}, lambdas_2_q={self.lambdas_2_q}, ablates={self.ablates})"

    def plotstr(self):
        return 'ConcaveSplitMethodDerandomizedPermute'

    def tabstr(self, adv):
        return f'concave_split_derandomized_permute_{adv}_d{self.dim}'

    def doubles_channels(self):
        return self.output_mode in ['center_center', 'center_error', 'upper_lower']

    def sample(self, x): # For training only, is random
        offset = torch.randint(self.levels,(x.shape[0],), device=self.device)
        level_ranges =self.all_lambdas[offset]
        split_values = (self.all_ss[offset] + 0.5)/255.0
    ...
    def sample_all_deterministic(self, x, start_sample = 0, end_sample = None): # Returns all possible distortions, output dim is (batch * distortions*dim)
        if (end_sample is None):
            end_sample = self.levels
        level_ranges =self.all_lambdas[start_sample:end_sample]
        split_values = (self.all_ss[start_sample:end_sample] + 0.5)/255.0
        ceil_dist = torch.ceil(255.*(x.unsqueeze(1)-split_values)/(level_ranges))*level_ranges/255. + split_values
        upper = torch.clamp(ceil_dist,max =1)
        lower = torch.clamp(ceil_dist - level_ranges/255. ,min =0)
        if self.output_mode == 'center':
...
class L_1_over_2_SplitMethodDerandomizedPermute(ConcaveSplitMethodDerandomizedPermute):
    def __init__(self, dim, seed = 0, s = 1., output_mode = 'center', device='cpu'):
        lambdas_2_q = [1]*17 + [4] * 17 + [14]*2 + [17,  23,  40,  47,  67, 169, 254, 255]
        ablates = 15 +  int(math.ceil(1000 * (s-1)))
        self.s = s
