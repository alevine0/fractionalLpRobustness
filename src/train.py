    ...
    for epoch in range(start_epoch, args.num_epochs):
        ...
        for i, (x, y) in enumerate(train_loader):
            ...
            if (noise.doubles_channels()):
                noise_shape = torch.Size([noise_shape[0],]) + torch.Size([noise_shape[1]*2,]) + noise_shape[2:]
            if args.adversarial:
                eps = min(args.eps, epoch * args.eps / (args.num_epochs // 2))
                x, loss = pgd_attack_smooth(model, x, y, eps, noise, sample_size=4, adv=args.adv)
            elif args.stability:
                x_tilde = noise.sample(x.view(len(x), -1)).view(noise_shape)
                x = noise.sample(x.view(len(x), -1)).view(noise_shape)
